      ...
      <View style={styles.chatInfo}>
        <View style={styles.chatHeader}>
          <View style={styles.nameRow}>
            <Text style={[styles.chatName, item.locked && styles.lockedName]}>{item.name}</Text>
            {item.badge && (
...
export default function ChatsScreen({ navigation }) {
  ...
  const handleChatPress = (item) => {
    if (item.locked) {
      ...
      navigation.navigate('ChatMessage', { name: item.name, avatar: item.avatar });
    }
  };
    ...
    <SafeAreaView style={styles.safe}>
      ...
      <View style={styles.searchContainer}>
        ...
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate('CIDFlow', { isAddContact: true })}
          activeOpacity={0.7}
        >
      ...
      <FlatList
        ...
        keyExtractor={(i) => i.id}
        renderItem={({ item }) => (
          <ChatRow item={item} onPress={() => handleChatPress(item)} navigation={navigation} />
        )}
